    ...
    const unsubscribeProgress = listen('image_optimization_progress', (event) => {
      ...
      if (processingRef.current) {
        ...
        if (isDetailedUpdate) {
          ...
          const { completedTasks, totalTasks, status, metadata } = event.payload;
          ...
          if (metadata && metadata.formattedMessage) {
            ...
            const savedBytesMatch = formattedMessage.match(/\(([\d.]+) (KB|MB|GB) saved/i);
            const compressionRatioMatch = formattedMessage.match(/\/ ([\d.]+)% compression/i);
            ...
            if (savedBytesMatch && compressionRatioMatch) {
              ...
              const estimatedOriginalSize = savedBytes / (compressionRatio / 100);
      ...
      if (event.payload.status === 'complete' && event.payload.metadata && event.payload.metadata.totalDuration) {
        // Use the backend's reported duration for accuracy
        const backendDuration = parseFloat(event.payload.metadata.totalDuration);
